...
function appendBackupContents(archive) {
  ...
  const messages = db.prepare('SELECT * FROM return_messages ORDER BY id ASC').all();
  ...
  archive.append(JSON.stringify(messages, null, 2), { name: 'data/return_messages.json' });
...
data/*.json      - every return, its status history, message threads, uploaded-file records, and staff accounts (no passwords)
